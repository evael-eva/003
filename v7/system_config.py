...
class SystemConfigDialog(QDialog):
    ...
    def _create_proxy_tab(self):
        ...
        info_text = QLabel(
            ...
            "2. 系统会定时从API提取新的代理IP\n"
            "3. 每次请求使用不同的代理IP\n"
            "4. 支持的返回格式：txt（每行一个IP:端口）\n\n"
            "示例API：\n"
            "https://share.proxy.qg.net/get?key=YOUR_KEY&num=50&format=txt"
        )
    ...
    def _test_proxy(self):
        ...
        # 从 API 提取代理
        try:
            proxies = self._extract_proxies_from_api(api_url, 1)
            if not proxies:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    QMessageBox.information(
                        ...
                        f"当前出口IP: {ip_info}\n"
                        f"使用代理: {test_proxy}\n\n"
                        f"注意: 此代理为HTTP代理，可用于爬取HTTP/HTTPS网站"
                    )
                else:
