    ...
    def _initialize_match_state(self, match_id, config, latest):
        ...
        try:
            ...
            if config.get('second_half_goal_enabled', False):
                ...
                # v12修复: 使用 .get() 安全访问，避免 KeyError
                if current_minute > 45 and state.get('second_half_baseline_goals') is None:
                    # v12关键修复: 基准值应该是进入下半场时的全场总进球数
                    # 而不是估算的上半场进球数
                    state['second_half_baseline_goals'] = total_goals  # ← 修复：使用total_goals
                    self.log_signal.emit(
                        f"[监控引擎] 比赛{match_id}初始化: 进入下半场，基准进球数={total_goals} "
                        f"(第{current_minute}分钟，全场{total_goals}球)"
                    )
                elif current_minute <= 45 and state.get('second_half_baseline_goals') is None:
                    # v12新增: 如果还在上半场，将基准值设为当前总进球数
                    # 这样下半场开始时，actual_second_half_goals = total - baseline = 0
                    state['second_half_baseline_goals'] = total_goals
                    self.log_signal.emit(
                        f"[监控引擎] 比赛{match_id}初始化: 上半场，预设基准进球数={total_goals} "
                        f"(第{current_minute}分钟，全场{total_goals}球)"
                    )
                ...
                # ⚠️ v14关键修复: 移除初始化时的立即触发逻辑
                # 原因：如果用户在上半场开始监控，此时不应该触发下半场规则
                # 下半场规则只应该在下半场有新进球时才触发
    ...
    def _check_all_rules(self, match_id, config, latest):
        ...
        try:
            ...
            if config.get('second_half_goal_enabled', False):
                ...
                # v14修复: 下半场规则只在下半场有效（>45分钟）
                # 如果还在上半场，不应该触发下半场规则
                if current_minute <= 45:
                    # v14新增: 在上半场时，预设基准值为当前总进球数
                    # 这样下半场开始时，actual_second_half_goals = total - baseline = 0
                    if state.get('second_half_baseline_goals') is None:
                        state['second_half_baseline_goals'] = total_goals
                        self.log_signal.emit(
                            f"[监控引擎] 比赛{match_id}在上半场，预设下半场基准进球数={total_goals} "
                            f"(第{current_minute}分钟，全场{total_goals}球)"
                        )
                    # ⚠️ v14关键修复: 上半场不检查下半场规则，跳过后续判断
                    # 但仍然需要更新 last_goals 状态
                    state['second_half_last_goals'] = 0
                else:
                    # v2优化: 记录下半场开始时的基准进球数
                    # v12修复: 使用 .get() 安全访问，避免 KeyError
                    ...
                    # v14修复: 只有在下半场且actual_second_half_goals > 0时才检查
                    # 使用实际的下半场进球数进行判断
                    # v7修复: 使用 .get() 安全访问，避免 KeyError
