            ...
            if self.half_ou_enabled:
                ...
                for match in filtered_matches:
                    ...
                    if mid in initial_odds_cache:
                        ...
                        if ou_half_initial:
                            # v12修复: 获取大球水位（over_odds），而不是盘口大小
...
class MainWindow(QMainWindow):
    ...
    def _create_settings_panel(self):
        ...
        watchdog_row.addWidget(QLabel("看门狗超时:"))
        ...
        self.watchdog_timeout_spin.setMinimumWidth(80)
        watchdog_row.addWidget(self.watchdog_timeout_spin)
        watchdog_row.addStretch()
        global_layout.addLayout(watchdog_row)
        
        # v7新增: 健康检查间隔
        health_check_row = QHBoxLayout()
        health_check_row.addWidget(QLabel("健康检查:"))
        ...
        self.health_check_interval_spin.setMinimumWidth(80)
        health_check_row.addWidget(self.health_check_interval_spin)
        health_check_row.addStretch()
        global_layout.addLayout(health_check_row)
        
        # v7新增: 内存阈值
        memory_row = QHBoxLayout()
        memory_row.addWidget(QLabel("内存阈值:"))
        ...
        self.memory_threshold_spin.setMinimumWidth(80)
        memory_row.addWidget(self.memory_threshold_spin)
        memory_row.addStretch()
        global_layout.addLayout(memory_row)
        ...
        # --- v8新增: 系统配置按钮（邮件+代理） ---
        config_btn = QPushButton("⚙️ 系统配置（邮件/代理）")
        config_btn.setMinimumHeight(45)
        config_btn.setFont(QFont("Microsoft YaHei", 13, QFont.Bold))
        config_btn.clicked.connect(self.on_open_system_config)
        layout.addWidget(config_btn)
        
        # 配置状态标签
        self.config_status_label = QLabel("📧 邮件: 未配置  |  🌐 代理: 未启用")
