...
class MonitorEngine(QThread):
    ...
    def _parse_first_half_goals(score_str, current_minute):
        ...
        ⚠️ 注意：这个方法已经不再使用，因为我们会从HTML中直接获取半场比分
        保留此方法仅作为后备方案
        
        策略：
        - 如果当前<=45分钟（含中场），全部进球算上半场
        - 如果当前>45分钟，无法精确区分上下半场，需要通过基准值计算
        ...
        if current_minute <= 45:
            ...
            # v12修复: 已经进入下半场，不能简单估算
            # 这个方法在下半场会被调用，但实际应该使用 goal_threshold_states 中的基准值
            # 这里返回一个保守估计，避免误判
            # 假设：如果总进球<=3，可能都是上半场进的；如果>3，至少有一部分是下半场
            if total <= 2:
                return total  # 少量进球很可能都是上半场的
            elif total <= 4:
                return total - 1  # 中等进球，假设下半场进了1个
            else:
                return total - 2  # 多进球，假设下半场进了2个
