    ...
    def _play_sound(alert_type):
        ...
        if _sys.platform == 'win32':
            ...
            try:
                ...
                print("[AlertService] winsound不可用")
            except Exception as e:
                print(f"[AlertService] 声音播放失败: {type(e).__name__}: {e}")
        else:
            # macOS / Linux: 使用系统 beep 或 afplay
            try:
                import subprocess
                if _sys.platform == 'darwin':
                    ...
                    if alert_type in ('goal_reached', 'first_half_goal', 'second_half_goal'):
                        ...
                        for _ in range(3):
                            subprocess.run(['afplay', '/System/Library/Sounds/Pop.aiff'], capture_output=True, timeout=3)
                            time.sleep(0.2)
                    else:
                        subprocess.run(['afplay', sound_file], capture_output=True, timeout=3)
                else:
                ...
                print(f"[AlertService] 声音播放失败(macOS): {type(e).__name__}: {e}")
