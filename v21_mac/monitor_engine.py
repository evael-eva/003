...
class MonitorEngine(QThread):
        ...
        while self._running:
            try:
                ...
                def _fetch_one(match_id):
                    # v21新增: 检查是否需要获取半场大小球数据
                    config = self.monitored_matches.get(match_id, {})
                    need_half_ou = config.get('half_time_odds_monitor_enabled', False)
                    return match_id, self.fetcher.fetch_latest_odds(match_id, need_half_ou=need_half_ou)
    ...
    def _check_all_rules(self, match_id, config, latest):
        ...
        try:
            ...
            # === v21新增规则: 上半场时间点大球水位监控 ===
            half_ou_latest = latest.get('half_ou_latest')
            if config.get('half_time_odds_monitor_enabled', False) and half_ou_latest:
                threshold_minute = config.get('half_time_odds_minute', 30)
                threshold_over_odds = config.get('half_time_odds_threshold', 0.85)
                
                # 检查是否到达设定时间
                if current_minute >= threshold_minute:
                    over_odds = self._safe_float(half_ou_latest.get('over_odds', ''))
                    if over_odds is not None:
                        op = config.get('half_time_odds_operator', '<')
                        if self._check_threshold(over_odds, threshold_over_odds, op):
                            op_txt = {'<': '低于', '>': '高于', '=': '等于'}.get(op, '<')
                            self._try_trigger(
                                match_id, 'half_time_odds_reached',
                                f"⏰ 上半场{threshold_minute}分钟大球水位达标! {config.get('home_team','')} vs {config.get('away_team','')}: "
                                f"比赛进行到第{current_minute}分钟，半场大球赔率={half_ou_latest.get('over_odds','')}"
                                f"(阈值{op_txt}{threshold_over_odds}), 盘口={half_ou_latest.get('goal_line', '')}"
                            )

            # === v21新增规则: 全场时间点大球水位监控 ===
            if config.get('full_time_odds_monitor_enabled', False) and ou_latest:
                threshold_minute = config.get('full_time_odds_minute', 70)
                threshold_over_odds = config.get('full_time_odds_threshold', 0.85)
                
                # 检查是否到达设定时间
                if current_minute >= threshold_minute:
                    over_odds = self._safe_float(ou_latest.get('over_odds', ''))
                    if over_odds is not None:
                        op = config.get('full_time_odds_operator', '<')
                        if self._check_threshold(over_odds, threshold_over_odds, op):
                            op_txt = {'<': '低于', '>': '高于', '=': '等于'}.get(op, '<')
                            self._try_trigger(
                                match_id, 'full_time_odds_reached',
                                f"⏰ 全场{threshold_minute}分钟大球水位达标! {config.get('home_team','')} vs {config.get('away_team','')}: "
                                f"比赛进行到第{current_minute}分钟，全场大球赔率={ou_latest.get('over_odds','')}"
                                f"(阈值{op_txt}{threshold_over_odds}), 盘口={ou_latest.get('goal_line', '')}"
                            )
