...
# === macOS 适配: 平台字体检测 ===
def _get_system_font(size=-1, bold=False):
    """获取跨平台系统中文字体（macOS使用PingFang SC，Windows使用Microsoft YaHei）"""
    from PyQt5.QtGui import QFont
    if sys.platform == 'darwin':
        family = "PingFang SC"
    else:
        family = "Microsoft YaHei"
    font = QFont(family, size if size > 0 else -1)
    if bold:
        font.setBold(True)
    return font
...
from system_config import SystemConfigDialog  # v8新增: 统一配置对话框
from profile_manager import ProfileManager
from monitor_status_panel import MonitorStatusPanel
...
class MatchMonitorWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        team_label.setFont(_get_system_font(13, bold=True))
        ...
        # --- 第三行: 全场进N球后无第(N+1)球提醒 ---
        row_t4 = QHBoxLayout()
        row_t4.setSpacing(8)
        self.full_match_no_goal_enabled_cb = QCheckBox("\u5168\u573a\u4ec5N\u7403")  # v16新增: 全场仅N球
        self.full_match_no_goal_enabled_cb.setChecked(False)
        self.full_match_no_goal_enabled_cb.setToolTip("\u5168\u573a\u8fdbN\u7403\u540e\uff0c\u5230\u8bbe\u5b9a\u65f6\u95f4\u4ecd\u672a\u8fdb\u7b2c(N+1)\u7403\u5219\u63d0\u9192")  # v16新增: 更新说明
        row_t4.addWidget(self.full_match_no_goal_enabled_cb)

        row_t4.addWidget(QLabel("\u8fdb\u7403\u6570:"))
        self.full_match_no_goal_threshold_spin = QSpinBox()
        self.full_match_no_goal_threshold_spin.setRange(0, 15)
        self.full_match_no_goal_threshold_spin.setValue(2)
        self.full_match_no_goal_threshold_spin.setSuffix(" \u7403")
        self.full_match_no_goal_threshold_spin.setMinimumWidth(70)
        self.full_match_no_goal_threshold_spin.setEnabled(False)
        row_t4.addWidget(self.full_match_no_goal_threshold_spin)

        row_t4.addWidget(QLabel("\u68c0\u6d4b\u65f6\u95f4:"))
        self.full_match_no_goal_minute_spin = QSpinBox()
        self.full_match_no_goal_minute_spin.setRange(60, 90)
        self.full_match_no_goal_minute_spin.setValue(80)
        self.full_match_no_goal_minute_spin.setSuffix(" \u5206\u949f")
        self.full_match_no_goal_minute_spin.setMinimumWidth(70)
        self.full_match_no_goal_minute_spin.setEnabled(False)
        row_t4.addWidget(self.full_match_no_goal_minute_spin)
        row_t4.addStretch()
        time_layout.addLayout(row_t4)

        # v16新增: 复选框控制两个输入框
        def toggle_full_match_inputs(enabled):
            self.full_match_no_goal_threshold_spin.setEnabled(enabled)
            self.full_match_no_goal_minute_spin.setEnabled(enabled)
        self.full_match_no_goal_enabled_cb.toggled.connect(toggle_full_match_inputs)

        main_layout.addWidget(group_time)

        # === v21新增分区: 时间点大球水位监控 ===
        group_odds_time = QGroupBox("⏰ 时间点大球水位监控")
        odds_time_layout = QVBoxLayout(group_odds_time)
        odds_time_layout.setSpacing(8)

        # --- 第一行: 上半场时间点大球水位监控 ---
        row_ot1 = QHBoxLayout()
        row_ot1.setSpacing(8)
        self.half_time_odds_monitor_enabled_cb = QCheckBox("上半场时间点")
        self.half_time_odds_monitor_enabled_cb.setChecked(False)
        self.half_time_odds_monitor_enabled_cb.setToolTip("到达设定时间且大球水位达到阈值时提醒（需要勾选才轮询半场大小球页面）")
        row_ot1.addWidget(self.half_time_odds_monitor_enabled_cb)

        row_ot1.addWidget(QLabel("检测时间:"))
        self.half_time_odds_minute_spin = QSpinBox()
        self.half_time_odds_minute_spin.setRange(1, 45)
        self.half_time_odds_minute_spin.setValue(30)
        self.half_time_odds_minute_spin.setSuffix(" 分钟")
        self.half_time_odds_minute_spin.setMinimumWidth(70)
        self.half_time_odds_minute_spin.setEnabled(False)
        row_ot1.addWidget(self.half_time_odds_minute_spin)

        row_ot1.addWidget(QLabel("大球水位:"))
        self.half_time_odds_threshold_spin = QDoubleSpinBox()
        self.half_time_odds_threshold_spin.setRange(0.01, 5.0)
        self.half_time_odds_threshold_spin.setValue(0.85)
        self.half_time_odds_threshold_spin.setSingleStep(0.05)
        self.half_time_odds_threshold_spin.setDecimals(2)
        self.half_time_odds_threshold_spin.setMinimumWidth(70)
        self.half_time_odds_threshold_spin.setEnabled(False)
        row_ot1.addWidget(self.half_time_odds_threshold_spin)

        row_ot1.addWidget(QLabel("比较:"))
        self.half_time_odds_operator_combo = self._make_operator_combo()
        self.half_time_odds_operator_combo.setEnabled(False)
        row_ot1.addWidget(self.half_time_odds_operator_combo)
        row_ot1.addStretch()
        odds_time_layout.addLayout(row_ot1)

        # v21新增: 复选框控制所有输入框
        def toggle_half_time_odds_inputs(enabled):
            self.half_time_odds_minute_spin.setEnabled(enabled)
            self.half_time_odds_threshold_spin.setEnabled(enabled)
            self.half_time_odds_operator_combo.setEnabled(enabled)
        self.half_time_odds_monitor_enabled_cb.toggled.connect(toggle_half_time_odds_inputs)

        # --- 第二行: 全场时间点大球水位监控 ---
        row_ot2 = QHBoxLayout()
        row_ot2.setSpacing(8)
        self.full_time_odds_monitor_enabled_cb = QCheckBox("全场时间点")
        self.full_time_odds_monitor_enabled_cb.setChecked(False)
        self.full_time_odds_monitor_enabled_cb.setToolTip("到达设定时间且大球水位达到阈值时提醒（使用现有全场大小球页面）")
        row_ot2.addWidget(self.full_time_odds_monitor_enabled_cb)

        row_ot2.addWidget(QLabel("检测时间:"))
        self.full_time_odds_minute_spin = QSpinBox()
        self.full_time_odds_minute_spin.setRange(45, 90)
        self.full_time_odds_minute_spin.setValue(70)
        self.full_time_odds_minute_spin.setSuffix(" 分钟")
        self.full_time_odds_minute_spin.setMinimumWidth(70)
        self.full_time_odds_minute_spin.setEnabled(False)
        row_ot2.addWidget(self.full_time_odds_minute_spin)

        row_ot2.addWidget(QLabel("大球水位:"))
        self.full_time_odds_threshold_spin = QDoubleSpinBox()
        self.full_time_odds_threshold_spin.setRange(0.01, 5.0)
        self.full_time_odds_threshold_spin.setValue(0.85)
        self.full_time_odds_threshold_spin.setSingleStep(0.05)
        self.full_time_odds_threshold_spin.setDecimals(2)
        self.full_time_odds_threshold_spin.setMinimumWidth(70)
        self.full_time_odds_threshold_spin.setEnabled(False)
        row_ot2.addWidget(self.full_time_odds_threshold_spin)

        row_ot2.addWidget(QLabel("比较:"))
        self.full_time_odds_operator_combo = self._make_operator_combo()
        self.full_time_odds_operator_combo.setEnabled(False)
        row_ot2.addWidget(self.full_time_odds_operator_combo)
        row_ot2.addStretch()
        odds_time_layout.addLayout(row_ot2)

        # v21新增: 复选框控制所有输入框
        def toggle_full_time_odds_inputs(enabled):
            self.full_time_odds_minute_spin.setEnabled(enabled)
            self.full_time_odds_threshold_spin.setEnabled(enabled)
            self.full_time_odds_operator_combo.setEnabled(enabled)
        self.full_time_odds_monitor_enabled_cb.toggled.connect(toggle_full_time_odds_inputs)

        main_layout.addWidget(group_odds_time)
    ...
    def get_config(self):
        ...
        cfg = {
            ...
            # --- v16新增: 全场进N球后无第(N+1)球提醒 ---
            'full_match_no_goal_enabled': self.full_match_no_goal_enabled_cb.isChecked(),
            'full_match_no_goal_threshold': int(self.full_match_no_goal_threshold_spin.value()),
            'full_match_no_goal_minute': int(self.full_match_no_goal_minute_spin.value()),

            # --- v21新增: 时间点大球水位监控 ---
            'half_time_odds_monitor_enabled': self.half_time_odds_monitor_enabled_cb.isChecked(),
            'half_time_odds_minute': int(self.half_time_odds_minute_spin.value()),
            'half_time_odds_threshold': round(self.half_time_odds_threshold_spin.value(), 2),
            'half_time_odds_operator': _op(self.half_time_odds_operator_combo),
            
            'full_time_odds_monitor_enabled': self.full_time_odds_monitor_enabled_cb.isChecked(),
            'full_time_odds_minute': int(self.full_time_odds_minute_spin.value()),
            'full_time_odds_threshold': round(self.full_time_odds_threshold_spin.value(), 2),
            'full_time_odds_operator': _op(self.full_time_odds_operator_combo),
...
class FilterWorkerThread(QThread):
    ...
    def run(self):
        """线程入口 - 执行完整筛选流程"""
        try:
            # v13新增: 连接 filter_ctrl 的日志信号到本线程的日志信号
            self.filter_ctrl.log_signal.connect(self.log_signal.emit)
            ...
            # v11新增: 过滤掉已结束的比赛（状态为“完场”、“结束”等）
            original_count = len(filtered_matches)
            ended_statuses = ['完', '完场', '结束', '已结束', 'FIN', 'FT']
            filtered_matches = [
                m for m in filtered_matches
                if m.get('status', '') not in ended_statuses
            ]
            filtered_out_count = original_count - len(filtered_matches)
            if filtered_out_count > 0:
                self.log_signal.emit(f"[v11] 已过滤 {filtered_out_count} 场已结束的比赛")
                self.progress.emit(f"已过滤 {filtered_out_count} 场已结束的比赛...")
            ...
            # 阶段4：批量获取半场初盘数据（v13优化: 使用带重试的批量方法）
            self.progress.emit(f"正在获取 {len(filtered_matches)} 场比赛半场初盘数据...")
            self.log_signal.emit("正在请求半场初盘数据...")
            
            half_time_results = self.fetcher.batch_fetch_half_time_initial(match_ids, max_retries=2)
            
            half_time_success = 0
            for mid, half_result in half_time_results.items():
            ...
            if self.half_ou_enabled:
                self.progress.emit("正在按半场大球初盘水位范围过滤...")
                self.log_signal.emit(f"应用半场大球初盘水位筛选: [{self.half_ou_min}, {self.half_ou_max}]")
                ...
                for match in filtered_matches:
                    ...
                    if mid in initial_odds_cache:
                        ...
                        if ou_half_initial:
                            # v13修复: 获取大球水位（over_odds），而不是盘口大小
                            ...
                            self.log_signal.emit(f"排除比赛{mid}: 无半场大小球初盘数据")
                    else:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # v8优化: 先加载代理配置，再初始化OddsFetcher
        proxy_config = self._load_proxy_config_from_file()
        self.fetcher = OddsFetcher(proxy_config if proxy_config and proxy_config.get('enabled') else None)
        
        # v8.3修复: 将fetcher传递给MonitorEngine，确保使用同一个实例
        self.monitor_engine = MonitorEngine(fetcher=self.fetcher)
        self.alert_svc = AlertService(log_dir='alert_logs')  # v2: 指定告警日志目录
        self.system_config_dialog = SystemConfigDialog(self)  # v8新增: 统一配置对话框
    ...
    def _load_proxy_config_from_file(self):
        """v8新增: 从文件加载代理配置"""
        try:
            ...
            if os.path.exists(config_file):
                with open(config_file, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                print(f"[启动] 已加载代理配置: enabled={config.get('enabled')}, api_url={config.get('api_url', '')[:50]}")
                return config
            else:
                print("[启动] 未找到代理配置文件")
                return None
        except Exception as e:
            print(f"[启动] 加载代理配置失败: {e}")
            return None
    ...
    def _init_ui(self):
        ...
        # v13优化: 禁用分割器手柄，防止用户拖动
        body_splitter.setHandleWidth(0)  # 隐藏分割线
        body_splitter.setChildrenCollapsible(False)  # 禁止折叠
        
        # v2优化: 适配高分辨率屏幕，增加面板宽度
        # 左侧：筛选面板（原280 -> 350）
        filter_panel = self._create_filter_panel()
        filter_panel.setMinimumWidth(320)
        filter_panel.setMaximumWidth(350)
        body_splitter.addWidget(filter_panel)
        
        # 中间：比赛数据区（弹性）
        body_splitter.addWidget(self._create_match_panel())
        
        # 右侧：设置面板（v13优化: 固定宽度，确保内容完整显示）
        settings_panel = self._create_settings_panel()
        settings_panel.setMinimumWidth(380)
        settings_panel.setMaximumWidth(450)
        body_splitter.addWidget(settings_panel)
        
        # v13优化: 设置各面板初始宽度比例
        body_splitter.setStretchFactor(0, 0)   # 左侧固定
        body_splitter.setStretchFactor(1, 1)   # 中间弹性
        body_splitter.setStretchFactor(2, 0)   # 右侧固定
                
        # v13优化: 根据窗口大小动态设置初始宽度
        initial_width = window_width
        left_width = 335   # 左侧固定335px
        right_width = 415  # 右侧固定415px
        center_width = max(600, initial_width - left_width - right_width - 40)  # 中间至少600px
    ...
    def _create_toolbar(self):
        ...
        title_label.setFont(_get_system_font(24, bold=True))  # v2优化: 22 -> 24
        ...
        self.toolbar_status.setFont(_get_system_font(15, bold=True))  # v2优化: 13 -> 15
        ...
        self.profile_combo.setFont(_get_system_font(12))
    ...
    def _create_filter_panel(self):
        ...
        range_row.addWidget(QLabel("最小水位:"))
        ...
        self.half_ou_max_spin.setSingleStep(0.05)
        ...
        # v13修复: 提示文字更新为水位
        hint_label = QLabel("💡 说明: 筛选半场大球水位在 [最小值, 最大值] 范围内的比赛")
        ...
        self.filter_btn.setFont(_get_system_font(16, bold=True))
    ...
    def _create_match_panel(self):
        ...
        header_title.setFont(_get_system_font(16, bold=True))
        ...
        config_title.setFont(_get_system_font(14, bold=True))
    ...
    def _create_settings_panel(self):
        ...
        # v13优化: 使用网格布局，2列排列，更紧凑
        row = 0
        
        # 刷新间隔
        global_layout.addWidget(QLabel("刷新间隔:"), row, 0)
        ...
        self.refresh_interval_spin.setMinimumWidth(100)
        global_layout.addWidget(self.refresh_interval_spin, row, 1)
        
        row += 1
        # 告警冷却期
        global_layout.addWidget(QLabel("告警冷却:"), row, 0)
        self.cooldown_spin = QSpinBox()
        self.cooldown_spin.setRange(10, 600)
        self.cooldown_spin.setValue(60)
        self.cooldown_spin.setSuffix(" 秒")
        self.cooldown_spin.setMinimumWidth(100)
        global_layout.addWidget(self.cooldown_spin, row, 1)
        
        row += 1
        # 看门狗超时
        global_layout.addWidget(QLabel("看门狗:"), row, 0)
        self.watchdog_timeout_spin = QSpinBox()
        self.watchdog_timeout_spin.setRange(30, 600)
        self.watchdog_timeout_spin.setValue(120)
        self.watchdog_timeout_spin.setSuffix(" 秒")
        self.watchdog_timeout_spin.setMinimumWidth(100)
        global_layout.addWidget(self.watchdog_timeout_spin, row, 1)
        
        row += 1
        # 健康检查
        global_layout.addWidget(QLabel("健康检查:"), row, 0)
        self.health_check_interval_spin = QSpinBox()
        self.health_check_interval_spin.setRange(30, 600)
        self.health_check_interval_spin.setValue(60)
        self.health_check_interval_spin.setSuffix(" 秒")
        self.health_check_interval_spin.setMinimumWidth(100)
        global_layout.addWidget(self.health_check_interval_spin, row, 1)
        
        row += 1
        # 内存阈值
        global_layout.addWidget(QLabel("内存阈值:"), row, 0)
        self.memory_threshold_spin = QSpinBox()
        self.memory_threshold_spin.setRange(100, 2000)
        self.memory_threshold_spin.setValue(500)
        self.memory_threshold_spin.setSuffix(" MB")
        self.memory_threshold_spin.setMinimumWidth(100)
        global_layout.addWidget(self.memory_threshold_spin, row, 1)

        layout.addWidget(global_group)

        # --- v13优化: 提醒开关（紧凑复选框）---
        alert_group = QGroupBox("🔔 提醒方式")
        alert_group.setStyleSheet(global_group.styleSheet())
        ...
        self.popup_cb = QCheckBox("弹窗提醒")
        self.popup_cb.setChecked(True)
        checkbox_layout.addWidget(self.popup_cb, 0, 0)

        self.sound_cb = QCheckBox("声音提醒")
        self.sound_cb.setChecked(True)
        checkbox_layout.addWidget(self.sound_cb, 0, 1)

        self.email_cb = QCheckBox("邮件提醒")
        self.email_cb.setChecked(False)
        checkbox_layout.addWidget(self.email_cb, 1, 0)

        self.one_shot_alert_cb = QCheckBox("单次告警")
        self.one_shot_alert_cb.setChecked(True)
        self.one_shot_alert_cb.setToolTip("每条规则触发后不再重复")
        checkbox_layout.addWidget(self.one_shot_alert_cb, 1, 1)
        
        alert_layout.addLayout(checkbox_layout)
        ...
        config_btn.setFont(_get_system_font(11, bold=True))
        ...
        """)
        config_btn.clicked.connect(self.on_open_system_config)
        layout.addWidget(config_btn)
        
        # 配置状态标签（更紧凑）
        self.config_status_label = QLabel("📧 未配置 | 🌐 未启用")
        ...
        log_group.setStyleSheet(global_group.styleSheet())
        log_layout = QVBoxLayout(log_group)
        log_layout.setContentsMargins(4, 4, 4, 4)
        
        self.log_text = QTextEdit()
        self.log_text.setReadOnly(True)
        self.log_text.setMaximumHeight(150)  # v13优化: 从220降到150
        ...
        # v13优化: 监控状态面板（放在最后）
        self.monitor_status_panel = MonitorStatusPanel(self)
        self.monitor_status_panel.remove_match_requested.connect(self.on_remove_match_from_status_panel)
        self.monitor_status_panel.log_message_requested.connect(self.add_log)
        layout.addWidget(self.monitor_status_panel)
    ...
    def _connect_signals(self):
        ...
        # v7新增: 监控状态面板信号
        self.monitor_engine.data_updated.connect(self._update_match_monitor_status)
        
        # v11新增: 比赛结束信号
        self.monitor_engine.match_ended_signal.connect(self.on_match_ended)
    ...
    def on_start_monitoring(self):
        ...
        # v7新增: 配置看门狗和健康检查
        self.monitor_engine.set_watchdog_timeout(self.watchdog_timeout_spin.value())
        self.monitor_engine.set_health_check_interval(self.health_check_interval_spin.value())
        self.monitor_engine.set_memory_threshold(self.memory_threshold_spin.value())
        ...
        # v7新增: 显示看门狗和健康检查配置
        self.add_log(f"  看门狗超时: {self.watchdog_timeout_spin.value()}秒")
        self.add_log(f"  健康检查: {self.health_check_interval_spin.value()}秒")
        self.add_log(f"  内存阈值: {self.memory_threshold_spin.value()} MB")
    ...
    def on_stop_monitoring(self):
        ...
        # v7新增: 清空监控状态面板
        if hasattr(self, 'monitor_status_panel'):
            self.monitor_status_panel.clear_all()
        if hasattr(self, '_monitor_status_tracker'):
            self._monitor_status_tracker.clear()
    ...
    def _update_match_monitor_status(self, match_id, latest_data):
        """
        v7新增: 更新某场比赛的监控状态
        每次数据更新时调用，用于维护监控状态面板
        :param match_id: 比赛ID
        :param latest_data: 最新数据（可能包含error）
        """
        from datetime import datetime
        ...
        match_name = "未知比赛"
        for row_idx, widget in self.match_widgets.items():
            if row_idx < len(self.filtered_matches):
                match_data = self.filtered_matches[row_idx]
        ...
        # 获取或初始化状态跟踪数据
        if not hasattr(self, '_monitor_status_tracker'):
            self._monitor_status_tracker = {}
        
        if match_id not in self._monitor_status_tracker:
            self._monitor_status_tracker[match_id] = {
        ...
        tracker = self._monitor_status_tracker[match_id]
        tracker['total_requests'] += 1
        tracker['last_update'] = datetime.now()
        
        # 判断是否成功
        has_error = latest_data.get('error') if latest_data else True
        if has_error:
            tracker['consecutive_failures'] += 1
        else:
            tracker['successful_requests'] += 1
            tracker['consecutive_failures'] = 0  # 重置连续失败计数
        
        # 计算成功率
        total = tracker['total_requests']
        success = tracker['successful_requests']
        ...
        self.monitor_status_panel.update_match_status(match_id, match_name, status_info)
    
    def on_remove_match_from_status_panel(self, match_id):
        """
        v7新增: 从监控状态面板请求移除比赛
        :param match_id: 比赛ID
        """
        ...
        if target_row is not None:
            # 取消勾选
            checkbox_item = self.match_table.item(target_row, 0)
            if checkbox_item:
                checkbox_item.setCheckState(Qt.Unchecked)
            
            # 触发on_checkbox_changed逻辑
            self.on_checkbox_changed(checkbox_item)
            
            self.add_log(f"[监控状态] 已移除比赛 ID:{match_id}")
        else:
            self.add_log(f"[监控状态] 未找到比赛 ID:{match_id}")
    ...
    def on_match_ended(self, match_id, home_team, away_team):
        """v11新增: 处理比赛结束事件
        1. 取消该比赛的勾选
        2. 将该比赛置底
        """
        try:
            ...
            target_row = None
            for row in range(self.match_table.rowCount()):
                item = self.match_table.item(row, 1)  # Col 1是match_id
                if item and item.text() == str(match_id):
                    target_row = row
                    break
            
            if target_row is None:
                self.add_log(f"[比赛结束] 未找到比赛{match_id}的表格行")
                return
            ...
            if checkbox_item:
                checkbox_item.setCheckState(Qt.Unchecked)
                self.add_log(f"[比赛结束] {home_team} vs {away_team} (ID:{match_id}) 已自动取消勾选")
            
            # 3. 将比赛置底（移动到最后一行）
            last_row = self.match_table.rowCount() - 1
            if target_row < last_row:
                # 保存该行的数据
                row_data = []
                for col in range(self.match_table.columnCount()):
                    item = self.match_table.item(target_row, col)
                    if item:
                        # 复制item的所有属性
                ...
                # 删除该行
                self.match_table.removeRow(target_row)
                
                # 在最后一行重新插入
                self.match_table.insertRow(last_row)
                for col, item in enumerate(row_data):
                    if item:
                        self.match_table.setItem(last_row, col, item)
                
                self.add_log(f"[比赛结束] {home_team} vs {away_team} 已移至列表底部")
            
            # 4. 从监控列表中移除（如果还在监控中）
            if match_id in self.monitored_match_ids:
                self.monitored_match_ids.discard(match_id)
                self.add_log(f"[比赛结束] {home_team} vs {away_team} 已从监控列表移除")
        
        except Exception as e:
            self.add_log(f"[比赛结束] 处理异常: {e}")
            import traceback
            traceback.print_exc()
    ...
    def on_load_profile(self):
        ...
        for row in checked_rows:
            ...
            try:
                ...
                # v16新增: 全场进N球后无第(N+1)球提醒
                widget.full_match_no_goal_enabled_cb.setChecked(profile_config.get('full_match_no_goal_enabled', False))
                widget.full_match_no_goal_threshold_spin.setValue(profile_config.get('full_match_no_goal_threshold', 2))
                widget.full_match_no_goal_minute_spin.setValue(profile_config.get('full_match_no_goal_minute', 80))
                
                # v21新增: 时间点大球水位监控
                widget.half_time_odds_monitor_enabled_cb.setChecked(profile_config.get('half_time_odds_monitor_enabled', False))
                widget.half_time_odds_minute_spin.setValue(profile_config.get('half_time_odds_minute', 30))
                widget.half_time_odds_threshold_spin.setValue(profile_config.get('half_time_odds_threshold', 0.85))
                
                widget.full_time_odds_monitor_enabled_cb.setChecked(profile_config.get('full_time_odds_monitor_enabled', False))
                widget.full_time_odds_minute_spin.setValue(profile_config.get('full_time_odds_minute', 70))
                widget.full_time_odds_threshold_spin.setValue(profile_config.get('full_time_odds_threshold', 0.85))
                ...
                # v2优化: 水位监控比较符设置
                self._set_combo_value(widget.asian_home_op, profile_config.get('asian_home_operator', '<'))
                self._set_combo_value(widget.asian_away_op, profile_config.get('asian_away_operator', '<'))
                self._set_combo_value(widget.ou_over_op, profile_config.get('ou_over_operator', '<'))
                self._set_combo_value(widget.ou_under_op, profile_config.get('ou_under_operator', '<'))
                
                # v21新增: 时间点大球水位监控比较符设置
                self._set_combo_value(widget.half_time_odds_operator_combo, profile_config.get('half_time_odds_operator', '<'))
                self._set_combo_value(widget.full_time_odds_operator_combo, profile_config.get('full_time_odds_operator', '<'))
    ...
    def on_open_system_config(self):
        """v8新增: 打开系统配置对话框"""
        result = self.system_config_dialog.exec_()
    ...
    def _apply_system_config(self):
        """v8新增: 应用系统配置"""
        # 1. 应用邮件配置
        email_config = self.system_config_dialog.get_email_config()
        if email_config.get('smtp_server') and email_config.get('sender_email'):
            # 配置AlertService
            self.alert_svc.configure_email(
                email_config.get('smtp_server', ''),
                int(email_config.get('smtp_port', 465)),
                email_config.get('sender_email', ''),
                email_config.get('sender_password', ''),
                email_config.get('receiver_emails', ''),
            )
            self.add_log("[邮件] 配置已应用")
        
        # 2. 应用代理配置
        proxy_config = self.system_config_dialog.get_proxy_config()
        self.add_log(f"[代理调试] 配置内容: enabled={proxy_config.get('enabled')}, api_url={proxy_config.get('api_url', '')[:50]}")
        
        if proxy_config.get('enabled'):
            self.fetcher.set_proxy(proxy_config)
            api_url = proxy_config.get('api_url', '')
            if len(api_url) > 40:
                api_display = api_url[:37] + "..."
            else:
                api_display = api_url
            self.add_log(f"[代理] 已启用: {api_display}")
            self.add_log(f"[代理] 当前代理: {self.fetcher.current_proxy if self.fetcher.current_proxy else '未获取'}")
            self.add_log(f"[代理] 轮换间隔: {self.fetcher.PROXY_ROTATE_INTERVAL}秒 (5分钟)")
        else:
            self.fetcher.set_proxy(None)
            self.add_log("[代理] 未启用")
    ...
    def _update_config_status(self):
        ...
        email_status = "✓ 已配置" if email_config.get('smtp_server') else "✗ 未配置"
        
        if proxy_config.get('enabled') and proxy_config.get('api_url'):
            # 显示API接口的一部分
            api_url = proxy_config.get('api_url', '')
            if len(api_url) > 30:
                api_display = api_url[:27] + "..."
            else:
                api_display = api_url
            proxy_status = f"✓ API: {api_display}"
        elif proxy_config.get('enabled'):
            proxy_status = "✓ 已启用"
        else:
            proxy_status = "✗ 未启用"
        
        self.config_status_label.setText(f"📧 邮件: {email_status}  |  🌐 代理: {proxy_status}")
    ...
    def on_test_email(self):
        ...
        if not email_config.get('smtp_server'):
            QMessageBox.warning(self, "警告", "请先在系统配置中设置邮件SMTP服务器")
            return
        
        # 使用系统配置对话框的测试方法
        ok, msg = self.system_config_dialog.send_test_email_from_config()
        if ok:
