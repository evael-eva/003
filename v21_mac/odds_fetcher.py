...
class OddsFetcher:
    ...
    def fetch_half_time_odds(self, match_id):
        """
        获取半场大小球实时数据（使用 changeDetail/handicapHalf.aspx）
        :param match_id: 比赛ID
        :return: {'header': [], 'rows': [], 'error': str or None}
        """
        url = f"https://vip.titan007.com/changeDetail/handicapHalf.aspx?id={match_id}&companyid={self.company_id}&l=0"
        # v7优化: 每次请求使用随机User-Agent
        headers = dict(self.session.headers)
        headers['User-Agent'] = self._get_random_user_agent()
        headers['Referer'] = f'https://vip.titan007.com/AsianOdds_n.aspx?id={match_id}&l=0'
        ...
        current_proxy = self.proxies.get('http', '无代理') if self.proxies else '无代理'
        display_proxy = current_proxy.split('@')[1] if '@' in current_proxy else current_proxy
        print(f"[请求开始] 比赛{match_id}(半场大小球实时) - 使用代理: {display_proxy}")
        
        try:
            response = self.session.get(url, timeout=15, headers=headers, proxies=self.proxies)
            if response.status_code != 200:
                print(f"[请求失败] 比赛{match_id}(半场大小球实时) - HTTP {response.status_code} - 代理: {current_proxy}")
                return {'header': [], 'rows': [], 'error': f'HTTP {response.status_code}'}

            print(f"[请求成功] 比赛{match_id}(半场大小球实时) - 代理: {current_proxy}")
            html_content = self._decode_response(response)
            return self._parse_overunder_table(html_content, f"半场大小球{match_id}")

        except requests.exceptions.ProxyError as e:
            print(f"[代理失效] 比赛{match_id}(半场大小球实时) - 代理 {current_proxy} 失效: {str(e)}")
            self._mark_proxy_failed(current_proxy)
            return {'header': [], 'rows': [], 'error': f'代理错误: {str(e)}'}
        except requests.exceptions.Timeout:
            print(f"[请求超时] 比赛{match_id}(半场大小球实时) - 代理: {current_proxy}")
            return {'header': [], 'rows': [], 'error': '请求超时'}
        except Exception as e:
            print(f"[请求异常] 比赛{match_id}(半场大小球实时) - 代理: {current_proxy} - 错误: {str(e)}")
            return {'header': [], 'rows': [], 'error': str(e)}

    def fetch_latest_odds(self, match_id, need_half_ou=False):
        """
        v12优化: 获取最新实时盘口数据（取第一行数据作为最新值）
        :param match_id: 比赛ID
        :param need_half_ou: 是否需要获取半场大小球数据（v21新增）
        :return: 同 fetch_initial_odds 格式，新增 half_time_score 和 half_ou_latest 字段
        """
        result = {
            'asian_latest': None,
            'ou_latest': None,
            'half_ou_latest': None,  # 新增: 半场大小球最新数据
            'asian_rows': [],
            'ou_rows': [],
        ...
        # v21新增: 根据需要获取半场大小球数据
        if need_half_ou:
            half_ou_data = self.fetch_half_time_odds(match_id)
            if not half_ou_data.get('error') and half_ou_data.get('rows'):
                latest = half_ou_data['rows'][0]  # 第一行是最新的
                result['half_ou_latest'] = self._extract_ou_row(latest)
